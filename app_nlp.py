...
st.markdown("""
    <style>
        [data-testid="stVerticalBlock"] > div:first-child > [data-testid="stHorizontalBlock"] {
            ...
            background-color: #f8f9fa; border-radius: 15px; border-left: 8px solid #1f77b4;
        }
...
def detect_double_negation(words):
    for i, w in enumerate(words):
        if w in NEGASI:
            ...
            if any(next_word in KATA_NEGATIF_PETUNJUK for next_word in window):
                return True
    return False
    ...
    if not user_input.strip():
        ...
        if any(link in user_input.lower() for link in ['http', 'www', '.com', 'wa.me']):
            ...
            if not kata_per_kata:
                ...
                if detect_double_negation(kata_per_kata):
                ...
                if { "tidak_bagus", "tidak_sesuai" } & kata_set:
                    punya_kata_negatif = True
                ...
                if punya_kata_negatif:
                    ...
                    if vectorizer is None or model is None:
                        sentiment_val, predict_val = "Error", "Model ML pkl gagal dimuat"
                    else:
                        try:
                            text_tfidf = vectorizer.transform([cleaned_text])
                            pred_str = str(model.predict(text_tfidf)[0]).strip().lower()
                            sentiment_val = "Positif" if pred_str in ['positive', 'positif'] else ("Negatif" if pred_str in ['negative', 'negatif'] else "Netral")
        ...
        if st.button("👎 Tidak Tepat", use_container_width=True):
            try:
                ...
                df_koreksi = pd.DataFrame([{
                    ...
                    "review_text": st.session_state.teks_asli,                  
                    "sentiment": label_koreksi,                                 
                    "teks_clean": st.session_state.hasil_analisis,
